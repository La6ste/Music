...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    ...
    if (user) {
      ...
      let button_public_invite = new MessageButton().setStyle('LINK').setLabel("Invite " + user.username).setURL("https://discord.com/api/oauth2/authorize?client_id=${user.id}&permissions=8&scope=bot%20applications.commands")
      ...
      message.reply({
        embeds: [new MessageEmbed()
          .setColor(ee.color)
          .setTitle(`Invite: L'a6sté | Music`)
          .setFooter(`${user.username} | Created by L'asté help militaro.dev`)
        ],
